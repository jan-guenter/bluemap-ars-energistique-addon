...
"""Generate the bounded deterministic Ars Énergistique review gallery."""
...
def build_function() -> bytes:
    ...
    for placement in cases.PLACEMENTS:
        ...
        inline_block_entity = (
            placement.block_state == "ae2:cable_bus"
            and placement.block_entity_snbt is not None
        )
        if inline_block_entity:
            block_state += placement.block_entity_snbt
        lines.extend(
            (
        ...
        if placement.block_entity_snbt is not None and not inline_block_entity:
            lines.append(
                f"data merge block {placement.x} {placement.y} {placement.z} "
                f"{placement.block_entity_snbt}"
            )
...
def verify_function() -> bytes:
    ...
    lines.append(
        "tellraw @a {\"text\":\"Ars Énergistique gallery checked\","
        "\"color\":\"green\"}"
    )
    return text_bytes("\n".join(lines))
...
def base_files() -> dict[Path, bytes]:
    ...
    return {
        ...
        Path("datapack/pack.mcmeta"): json_bytes(
            {
                "pack": {
                    "description": "ATM 1.2.0 Ars Énergistique review gallery",
                    "pack_format": 48,
                }
            }
        ),
        ...
        function_root / "load.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            "tellraw @a {\"text\":\"Ars Énergistique gallery loaded; "
        ...
        function_root / "tp.mcfunction": text_bytes(
            "# Generated by gallery/generate.py; do not edit.\n"
            f"tp @s {cases.TELEPORT[0]} {cases.TELEPORT[1]} {cases.TELEPORT[2]} "
            f"{cases.TELEPORT[3]} {cases.TELEPORT[4]}"
        ),
